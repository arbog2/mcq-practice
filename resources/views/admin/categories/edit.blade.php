...
@section('content')
    <div class="card stack" style="max-width:720px;">
        <h1>编辑分类：{{ $category->name }}</h1>

        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="stack">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input id="sort_order" type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" min="0">
