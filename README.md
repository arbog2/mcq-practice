...
### 学员端
- **试卷练习** — 管理员组卷后学员选择试卷作答，选项内容打乱后按 A/B/C/D 顺序显示
- **双模式作答** — 支持"一页全显示"和"一页一题"两种模式，可随时切换
  - 一页一题模式：题号宫格（绿=未答，红=已答），上一题/下一题/提交按钮
- **自动评分** — 提交后即时评分，展示全部选项、正确答案和解析
- **错题本** — 自动记录错题，可按分类筛选
- **错题重练** — 从错题中抽取题目重新练习，答对自动标记已掌握
  - 整体抽取：从所有错题中随机抽题
  - 分类抽取：从指定分类的错题中抽题
- **练习历史** — 查看试卷练习和错题重练的历史记录与成绩
- **个人信息编辑**
...
- **题库管理** — 题目的增删改查
  ...
  - 每页条数选择（20/40/80/100）
  - 支持从 Excel 导入题目
- **试卷管理** — 创建/编辑试卷，从题库搜索题目组卷
  - 按分类筛选 + 关键词搜索题目
  - 分页浏览题库，勾选加入试卷
  - 查看学员成绩，按用户分类筛选，支持 CSV 导出
- **分类管理** — 分类的增删改查，支持排序
...
- **系统设置**
  ...
  - 注册是否需要审核
  - 每次练习的题量
- **错题统计** — 按用户分类 × 题库分类 × 题目统计错误次数，数据来源涵盖分类练习和试卷练习，支持 CSV 导出
